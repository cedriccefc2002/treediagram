...
export interface ITreeListProps {
    GotoEditHandler: (uuid: string, type: TreeDiagram_Tree.TreeType) => void;
}
...
export class TreeList extends React.Component<ITreeListProps, ITreeListState> {
    ...
    public renderTress() {
        ...
        for (const tree of this.state.trees) {
            ...
            const editfn = () => { this.props.GotoEditHandler(tree.uuid, tree.type); };
            rows.push(<tr key={i}>
                <th scope="row">{i}</th>
                <td>{tree.uuid}</td>
                <td>{tree.type.toString() === "Binary" ? "二元樹" : "普通樹"}</td>
                <td>
                    <button type="button" className="btn btn-success" onClick={editfn}>編輯</button>
                </td>
                <td>
